...
Write-Host "🖼️ MANUAL IMAGE UPLOAD TEST" -ForegroundColor Cyan
...
if (Test-Path "uploads") {    $existingFiles = Get-ChildItem "uploads" -File
    ...
    Write-Host "ℹ️ Uploads directory will be created automatically" -ForegroundColor Yellow
}

# Check database schema
Write-Host ""
Write-Host "🗄️ Database Schema Verification..." -ForegroundColor Yellow
...
Write-Host "   a) Click the '✏️ Edit' button on any item" -ForegroundColor Gray
...
# Cleanup option
Write-Host ""
Write-Host "🧹 To cleanup test files after testing:" -ForegroundColor Yellow
